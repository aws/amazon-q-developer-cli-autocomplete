...
impl KnowledgeSubcommand {
    /// Execute the knowledge subcommand - Clean orchestration
    ...
    async fn execute_operation(&self, ctx: &Context, output: &mut impl Write) -> OperationResult {
        match self {
            ...
            KnowledgeSubcommand::Clear => Self::handle_clear().await,
            KnowledgeSubcommand::Status => Self::handle_status().await,
            KnowledgeSubcommand::Cancel { operation_id } => Self::handle_cancel(operation_id).await,
    ...
    async fn handle_clear() -> OperationResult {
        let knowledge_store = KnowledgeStore::get_instance();
        let mut store = knowledge_store.lock().await;

        match store.clear().await {
            Ok(count) => OperationResult::Success(format!("Cleared {} entries from knowledge base", count)),
    ...
    async fn handle_cancel(operation_id: &str) -> OperationResult {
        let knowledge_store = KnowledgeStore::get_instance();
        let mut store = knowledge_store.lock().await;
...
<cyan!>Available commands</cyan!>
  ...
  <em>cancel OPERATION_ID</em>        <black!>Cancel a background operation</black!>
...
<cyan!>Examples</cyan!>
  ...
  /knowledge cancel op_123456           Cancel a background operation
"#
        )
    }
}
