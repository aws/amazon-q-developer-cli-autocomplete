        ...
        loop {
            ...
            let result = match chat_state {
                ...
                ChatState::HandleInput {
                    ...
                    tokio::select! {
                        res = self.handle_input(telemetry, input, database, tool_uses, pending_tool_index) => res,
    ...
    async fn handle_input(
        ...
        Ok(match command {
            Command::Knowledge { subcommand } => {
                // Check if Knowledge feature is enabled in settings
                if !database.settings.get_bool(Setting::EnabledKnowledge).unwrap_or(false) {
                    queue!(
                        self.output,
                        style::SetForegroundColor(Color::Red),
                        style::Print(
                            "\nError: Unknown command: '/knowledge'. Type '/help' to see available commands.\n\n"
                        ),
                        style::SetForegroundColor(Color::Reset)
                    )?;
